...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    window.addCarriage = function () {
      if (carriageCount < 16) {
        // เช็คว่ายังมีที่ว่างในรถไฟหรือไม่ (เช่นให้มีขีดจำกัดที่ 5 โบกี้)
        carriageCount++;
        ...
        alert("ไม่สามารถเพิ่มโบกี้ได้อีก เพราะมีโบกี้ครบแล้ว");
      }
    };
    ...
    train.addEventListener('drop', (e) => {
        ...
        train.addEventListener("drop", (e) => {
          ...
          if (dragged && e.target.className === "carriage") {
            ...
            showCarriagePosition(dragged); // เรียกใช้ฟังก์ชันเพื่อแสดงตำแหน่งของโบกี้
          }
        });
    });
...
// เพิ่มฟังก์ชันสำหรับแสดงตำแหน่งของ Carriage
function showCarriagePosition(carriage) {
  const carriageId = carriage.id;
  const carriagePosition = Array.from(train.children).indexOf(carriage) + 1;
  alert(`โบกี้ ${carriageId} อยู่ที่ตำแหน่งที่ ${carriagePosition}`);
}
});
